    ...
    try {
      ...
      const html = `
        ...
        <body style="font-family: Arial; padding: 20px;">
          ...
          <h2>Next Steps</h2>
          ${(report?.next_steps || []).map(i => `<p>• ${i}</p>`).join("")}

          <h2>Diet Recommendations</h2>
          <h3>Recommended</h3>
          ${(report?.diet?.good || []).map(i => `<p>• ${i}</p>`).join("")}

          <h3>Avoid</h3>
          ${(report?.diet?.avoid || []).map(i => `<p>• ${i}</p>`).join("")}

          <h2>Exercise</h2>
          ${(report?.exercise || []).map(i => `<p>• ${i}</p>`).join("")}

          <h2>Prevention Tips</h2>
          ${(report?.prevention || []).map(i => `<p>• ${i}</p>`).join("")}
